...
# Composition matrix
dir()
composition <- read.csv2("anura_islands_without0.csv")
# put names in rows
composition <- composition %>% select(-X.1, -X)
rownames(composition) <- composition$id 
...
names_islands <- as_tibble(cbind(v1=shape$OBJECTID_1, v2=shape$Name_USGSO))
grids <- names_islands %>%  
  mutate(grids = paste0(v1,"_",v2)) 
...
phyloregions.shape <- phyloregion(beta_diversity[[1]], k = 19, method = "average", shp= shape)      

plot_NMDS(phyloregions, cex=6)
text_NMDS(phyloregions, cex=2)
plot(phyloregions, cex=1, palette="NMDS")
